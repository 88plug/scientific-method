...
# spec defines no root-level plugin.json; a stray one orphans hooks, so reject it.
if (ROOT / "plugin.json").exists():
    err(
        "root plugin.json must not exist (the spec defines none; "
        "the only manifest is .claude-plugin/plugin.json)"
    )
...
if not man.exists():
    ...
    try:
        ...
        kw = m.get("keywords", [])
        if len(kw) != 20:
            err(f"plugin.json: keywords must be exactly 20 (found {len(kw)})")
...
for md in list(ROOT.glob("skills/**/SKILL.md")) + list(ROOT.glob("agents/**/*.md")):
    ...
    if e:
        err(f"{rel(md)}: {e}")
    elif d is None or not d.get("name") or not d.get("description"):
        err(
...
for md in list(ROOT.glob("commands/**/*.md")):
    ...
    if e:
        err(f"{rel(md)}: {e}")
    elif d is None or not d.get("description"):
